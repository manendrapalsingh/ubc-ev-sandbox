...
# Publish all BAP test messages to Kafka
...
#   ./publish-all.sh discover      # Publish only discover messages
#   ./publish-all.sh select        # Publish only select message
...
# BAP Backend publishes to bap.* topics (requests from BAP Backend)
# The BAP plugin's bapTxnCaller consumes from bap.* topics
get_topic() {
  local action=$1
  case "$action" in
    discover)
...
get_action_from_file() {
  ...
  if command -v jq &> /dev/null; then
    ...
    basename "$json_file" .json | sed 's/^discover-.*/discover/' | sed 's/-.*$//'
  fi
}
...
echo "=========================================="
echo "BAP Kafka Message Publisher"
echo "=========================================="
