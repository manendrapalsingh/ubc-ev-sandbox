# API Integration - REST/HTTP Communication

This guide demonstrates how to integrate the **onix-adapter** with BAP and BPP applications using **Docker containers** with **REST API** communication.

## Architecture Overview

The onix-adapter runs as a container service that handles both incoming and outgoing API requests. All communication happens via HTTP/REST API endpoints.

### Components

- **Redis**: Used for caching and state management
- **Onix-Adapter**: Container handling all BAP/BPP operations
...
- Schema files from `../../schemas` directory (read-only, mounted from repository root)
...
**Note**: 
- Config paths are relative to the `onix-adaptor/api/` directory
- Schema path `../../schemas` points to the root-level `schemas/` directory containing `beckn.one_deg_ev-charging/v2.0.0/` schema files
...
- For complete sandbox environment: See [HTTP/REST Sandbox](../../sandbox/api/README.md)
- For Kafka sandbox environment: See [Kafka Sandbox](../../sandbox/kafka/README.md)
