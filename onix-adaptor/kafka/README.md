...
This guide demonstrates how to integrate the **onix-adapter** with BAP and BPP applications using **Docker containers** with **Apache Kafka** event streaming. The same configuration works for both monolithic and microservice architectures and now uses Kafka’s **KRaft** mode (no ZooKeeper dependency).
...
- **Apache Kafka (KRaft)**: Event streaming platform for high-throughput messaging
...
├── docker-compose-onix-bap-kafka-plugin.yml    # BAP service configuration (KRaft)
├── docker-compose-onix-bpp-kafka-plugin.yml    # BPP service configuration (KRaft)
└── README.md
```
...
2. **Verify services are running:**
   ```bash
   docker ps | grep -E "(redis-bap|onix-bap-kafka-plugin|kafka)"
   ```

3. **Check logs:**
   ```bash
   docker-compose -f docker-compose-onix-bap-kafka-plugin.yml logs -f onix-bap-kafka-plugin
   ```
...
2. **Verify services are running:**
   ```bash
   docker ps | grep -E "(redis-bpp|onix-bpp-kafka-plugin|kafka-bpp)"
   ```

3. **Check logs:**
   ```bash
   docker-compose -f docker-compose-onix-bpp-kafka-plugin.yml logs -f onix-bpp-kafka-plugin
   ```
...
1. **Kafka (KRaft)**: Event streaming platform (defaults: `kafka:9092` for BAP, `kafka-bpp:9092` for BPP)
...
- `CONFIG_FILE`: Path to the adapter configuration file
  - BAP: `/app/config/adapter.yaml` (mounted from `config/onix-bap/adapter.yaml`)
  - BPP: `/app/config/adapter.yaml` (mounted from `config/onix-bpp/adapter.yaml`)
...
A fully-wired sandbox that mirrors this setup is available under `sandbox/kafka/`.  
It adds the supporting mock services (CDS plus mock BAP/BPP backends) together with ready-to-use Kafka
message publishers. Key locations:

- `sandbox/kafka/docker-compose.yml` — spins up Kafka (KRaft), both ONIX plugins and supporting mocks
- `sandbox/kafka/mock-*.yml` — configuration for the mock BAP and BPP Kafka services alongside CDS
- `sandbox/kafka/message/bap|bpp/` — sample payloads plus helper scripts (`publish-all.sh`,
...
> sandbox compose stack is up (`docker compose up -d` from `sandbox/kafka/`).

For detailed sandbox documentation, see: **[Kafka Sandbox Guide](../../sandbox/kafka/README.md)**
...
```bash
# List topics (BAP stack)
docker exec kafka kafka-topics.sh --list --bootstrap-server localhost:9092

# List topics (BPP stack)
docker exec kafka-bpp kafka-topics.sh --list --bootstrap-server localhost:9092
...
- Verify Kafka is running: `docker ps | grep -E "(kafka$|kafka-bpp)"`
- Check Kafka logs: `docker logs kafka` (BAP) or `docker logs kafka-bpp` (BPP)
...
- Review adapter logs: `docker logs onix-bap-kafka-plugin` or `docker logs onix-bpp-kafka-plugin`
...
1. **No Consumers Connected**:
   - Check adapter logs: `docker logs onix-bap-kafka-plugin` or `docker logs onix-bpp-kafka-plugin`
...
- For API integration: See [API Integration Guide](../api/README.md)
- For complete Kafka sandbox environment: See [Kafka Sandbox Guide](../../sandbox/kafka/README.md)
