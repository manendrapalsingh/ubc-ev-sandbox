...
- `CONFIG_FILE`: Path to the adapter configuration file
  - BAP: `/app/config/adapter.yaml`
  - BPP: `/app/config/adapter.yaml`
- `REDIS_PASSWORD`: Redis password for authentication (default: `your-redis-password`)
...
- **Routing Keys** (callbacks from BPP Backend): 
  ...
  - `bpp.catalog_publish` - Catalog publish request
  - `bpp.on_catalog_publish` - Catalog publish callback
- **Acknowledgment**: Manual (`autoAck: false`)
- **Prefetch Count**: 10
- **Consumer Threads**: 2
...
- **Publishing Routing Keys** (published by `bppTxnReceiver`):
  - `bpp.discover`, `bpp.select`, `bpp.init`, `bpp.confirm`, `bpp.status`, `bpp.track`, `bpp.cancel`, `bpp.update`, `bpp.rating`, `bpp.support`, `bpp.catalog_publish`, `bpp.on_catalog_publish`
...
1. **Verify config files are mounted**:
   ...
   docker exec onix-bap-plugin-rabbitmq ls -la /app/config/
   
   # Check routing configurations
   docker exec onix-bap-plugin-rabbitmq cat /app/config/adapter.yaml
   ```
